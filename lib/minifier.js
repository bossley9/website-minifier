...
module.exports.inlineJS = function(rootDir, min) {
  ...
  // 2. ends with "</script>"
  let scripts = min.match(/<script\s*.*?\s*<\/script>/gmi);
  let src = "", srcRaw = [];
  for (let i = 0; i < scripts.length; i++) {
    ...
    if (scripts[i].indexOf("src") > -1) {
      ...
      let jsMinified = this.minifyCode(js);
    ...
    // if no external source exists
      ...
      let jsScriptMinified = this.minifyCode(jsScript);
...
module.exports.genFiles = function(filePath, callback) {
  ...
  // begin minifying
  // replace: remove inline single-line js comments to prevent bugs later
  htmlString = this.stringifyFile(filePath).replace(/\/\/.*/gi, "");
